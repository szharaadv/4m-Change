...
// NOTE: recipient selection is driven by the department routing tables
// (department_managers / department_qc) via getDeptManagers()/getDeptQc()
// in helpers/common.php — not by role name. The helpers below are kept
// for backward compatibility and are not used by the current workflow.
function getManagerEmails(PDO $pdo, string $category = ''): array
{
    if ($category) {
        ...
        if (empty($result)) {
            return $pdo->query("SELECT u.name, u.email FROM users u JOIN department_managers dm ON dm.manager_id = u.id WHERE u.email != '' AND u.is_active = 1 GROUP BY u.id")->fetchAll();
        }
...
{
    return $pdo->query("SELECT u.name, u.email FROM users u JOIN department_qc dq ON dq.qc_id = u.id WHERE u.email != '' AND u.is_active = 1 GROUP BY u.id")->fetchAll();
}
